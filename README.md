...
- **Выход**: Для каждой рамки SSD предсказывает класс и сдвиги, всего \($Num bounding boxes \times (4 + C + 1)$ \) предсказаний.
...
DETR использует венгерский алгоритм для сопоставления предсказанных рамок (\( Num queries \)) с ground-truth (\( M \), где \( $M \leq Num quries$ \)). Алгоритм минимизирует комбинированную потерю:
